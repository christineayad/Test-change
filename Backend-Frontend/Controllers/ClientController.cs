...
namespace Backend_Frontend.Controllers
{
    public class ClientController : Controller
    {
        ...
        public ActionResult Index(int page=1, int size=3, string searchterm = "")
        {
           
            ViewBag.page = page; 
            ViewBag.size=size; 
            ViewBag.searchterm = searchterm;
            IEnumerable<ClientStatusVM> clientAll = _client.GetAllClient(page, size, searchterm);

            if(!string.IsNullOrEmpty(searchterm))
            {
                return PartialView("_ClientsTablePartial", clientAll);
        ...
        //        return PartialView("_SearchIndex", clientVM);
